...
const PerformanceSection = () => {
  ...
  return (
    <div className="performance-slider-container py-10 bg-background flex flex-col items-center pb-20">
      ...
      <div className="performance-slider " style={{ transform: `translateX(-${currentPerformance * 25}%)` }}>
        {performances.map((performance, index) => (
          <div key={performance.id} className="bg-white performance-card border flex flex-col items-center justify-center m-2 p-0">
            <img src={performance.image} alt={`Performance ${performance.id}`} width={500} className="aspect-auto" />
            <div className="performance-details ">
              <div className="font-bold text-xl my-2">Performance {performance.id}</div>
