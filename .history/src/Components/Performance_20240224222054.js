...
const PerformanceSection = () => {
    ...
    return (
        <section id='performance-slider-container' className="performance-slider-container bg-background">
            <div>
            <h2 className='text-5xl text-primary font-semibold'>सादरीकरणे</h2>
            <div className="performance-slider my-10" style={{ transform: `translateX(-${currentPerformance * 26}%)` }}>
                {performances.map((performance, index) => (
                    <div key={performance.id} className="bg-white h-96 performance-card border flex flex-col items-center justify-start m-2 p-0">
                        <img src={performance.image} alt={`Performance ${performance.id}`} width={500} className="aspect-video" />
                        <div className="performance-details flex flex-col justify-center items-center ">
